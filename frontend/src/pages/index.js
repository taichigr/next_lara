...
const Home = () => {
  ...
  useEffect(() => {
    threads &&
      setResponses(threads.map(() => ({ name: "", email: "", content: "" })));
  }, [threads]);
              ...
              <div className="p-2 border border-black">
                ...
                <div className="px-8">
                  {responses[index] && (
                    <form
                      className="mb-2"
                      onSubmit={(e) => createResponse(e, thread.id, index)}
                    >
                  ...
                  <Link href={`/threads/${thread.id}`}>
                    <p className="text-sm text-blue-900 underline">全部読む</p>
                  </Link>
                </div>
              </div>
            </div>
      ...
      <section className="p-1.5 bg-green-200">
        <div className="py-1 text-center border border-black">
          <Link href="/threads/new">
            <p className="px-1.5 py-0.5 text-sm bg-gray-300 border border-gray-500 rounded">
              新規スレッド書き込み画面へ
            </p>
          </Link>
        </div>
